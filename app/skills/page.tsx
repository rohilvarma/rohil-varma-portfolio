...
const Skills = () => {
  ...
  const renderLanguages = () => {
    ...
    return languages.map((language) => <SkillItem skillItem={language} key={language.key} />);
  };

  const renderFrameworks = () => {
    const frameworks = skills.filter((skill) => skill.type === "framework");
    return frameworks.map((framework) => <SkillItem skillItem={framework} key={framework.key} />);
  };

  const renderClouds = () => {
    const clouds = skills.filter((skill) => skill.type === "cloud");
    return clouds.map((cloud) => <SkillItem skillItem={cloud} key={cloud.key} />);
  };
  return (
    <div className="">
